...
-----------
	...
	作为替换，任务设计人员可以使用ePL的REPEAT函数。
	
	任务设计人员需要定义特定的验证逻辑，这样在最大执行时间（WCET）内，
	可以确保Miner提交的悬赏和POW的解决方案可以被分布式网络内的所有节点所验证。
...
----------------------
        ...
        ePL能够存储有限的数据，用于初始化算法的后续迭代。这允许任务设计人员从同一个工作包迭代前，去创建算法并建立公认的任务悬赏。
...
-----------------
	...
	----------------
                ...
                所有的ePL程序必须有一个'verify_pow'表达式，用于检查任务设计人员产生的4个无符号整数型的MD5哈希值是否小于当前的POW目标值。
	        'verify_pow'表达式的格式如下：
